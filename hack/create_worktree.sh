...
if [ "$INIT_THOUGHTS" = true ]; then
    ...
    cd "$WORKTREE_PATH"
    if silmari-oracle init --directory humanlayer > /dev/null 2>&1; then
        echo "✅ Thoughts initialized!"
        # Run sync to create searchable directory
        if silmari-oracle sync > /dev/null 2>&1; then
            echo "✅ Thoughts searchable index created!"
        else
            echo "⚠️  Could not create searchable index. Run 'silmari-oracle sync' manually."
        fi
    else
        echo "⚠️  Could not initialize thoughts automatically. Run 'silmari-oracle init' manually."
    fi
fi
